...
class __crack__:
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            print '\n [%s+%s] Unfortunately open now -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print ' [%s+%s] securitycheck open agter 7 days -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print '\n [%s!%s] crack process\n'%(M,N)
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                                    try:
    ...
    def __pler__(self):
        yan = raw_input('\n [*] method : ')
        if yan == '':
            print '\n %s[%s×%s] bro'%(N,M,N);self.__pler__()
        elif yan in ('1', '01'):
            print '\n [%s+%s]  security check IDs open after 7 days -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s]  unfortunately open now -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] Just wait and watch the result\n'%(M,N)
            with Black(max_workers=30) as (__yayanXD__):
            ...
            print '\n [%s+%s]  unfortunately open now -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] security check IDs open after 7 days -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] Just wait and watch the hacking \n'%(M,N)
            with YayanGanteng(max_workers=30) as (__yayanXD__):
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        name, uid = yntkts.split('<=>')
            ...
            print '\n [%s+%s] unfortunately open now -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] security check IDs open after 7 days -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] Hacking done open Ids after 7 to 8 days\n'%(M,N)
            with YayanGanteng(max_workers=30) as (__yayanXD__):
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        name, uid = yntkts.split('<=>')
